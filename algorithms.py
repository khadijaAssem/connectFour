...
class Minimax_Class:
    ...
    def __init__(self, num_row, num_col, k_levels, opponent, agent, default, withPruning):
    ...
    def getStep(self, states):
        tree1 = {}
        res, row, col = self.minimax(0, 0, True, states, tree1)
    ...
    def minimax (self, current_depth, nodeIndex, is_max, states, tree, alpha = None, beta=None):
        if alpha == None:
            alpha = self.alpha
            beta = self.beta
        # base case : targetDepth reached
        if (current_depth == self.k_levels):  # call Heuristic
        ...
        if (is_max):
            ...
            for element in range(len(new_list)):
                ...
                temp_max,w,z = self.minimax(current_depth + 1, x, False, states, tree, alpha, beta)
            ...
            for element in range(len(new_list)):
                x = num_node_prev_level + element
                temp_min,w,z = self.minimax(current_depth + 1, x, True, states, tree, alpha, beta)
